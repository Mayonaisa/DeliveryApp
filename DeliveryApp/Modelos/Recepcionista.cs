...
namespace DeliveryApp.Modelos
{
    public class Recepcionista : Usuario
    {
        ...
        public void RecepMasRegistros(ref int Registros)
        {
            SqlConnection conx = new SqlConnection(ConfigurationManager.ConnectionStrings["conx"].ConnectionString);
            ...
            SqlCommand consulta = new SqlCommand("SELECT top 1 R.idRecepcionista,P.nombre,P.aPaterno,P.aMaterno, COUNT(*) as 'Registros' FROM Registra Re, Recepcionista R, Persona P WHERE R.idRecepcionista = Re.idRecepcionista and P.idPersona = R.idRecepcionista and cast(left(Re.fechaRegistro, LEN(Re.fechaRegistro) - 15) as date) > DATEADD(MONTH, -1, GETDATE()) group by R.idRecepcionista, p.nombre, P.aPaterno, P.aMaterno", conx);
            ...
            if (resultado.Read())
            {
                ...
                Registros =int.Parse(resultado.GetValue(4).ToString());
            }
